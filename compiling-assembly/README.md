...
nasm -f elf file.asm
```
this will take our file.asm with our assembly code and produce and object file called `file.o`. we are also clearly asking for an elf file.
...
here, `elf_i386` as we are doing 32-bit executables, is the format we want the exec file to be in, `file.o` the path to the object file and `file` the name of the executable we wish to create. 
